...
const makeTeam = (c) => Array.from({length:20}, (_,i) => `${c}${i+1}`);

// Mapa ISO para bandeiras reais
const ISO_MAP = {
...
const SPECIAL = {FWC:"🏆", CC:"🥤", EXTRA:"⭐", ES:"⭐"};
const flag = (code, size="32x24") => {
...
const COCA  = { id:"CC",    label:"Coca-Cola — Figurinhas Especiais",         color:"#ef4444", group:null, stickers:Array.from({length:14},(_,i)=>`CC${i+1}`) };
const EXTRA = { id:"EXTRA", label:"Extra Stickers (Legends) — fora do álbum", color:"#a855f7", group:null, stickers:Array.from({length:20},(_,i)=>["R","B","S","G"].map(v=>`ES${i+1}${v}`)).flat() };
const ALL_DATA      = [...ALBUM_OFICIAL, COCA, EXTRA];
const ALL_CODES     = ALL_DATA.flatMap(s => s.stickers);
const TOTAL_OFICIAL = ALBUM_OFICIAL.reduce((a,s) => a+s.stickers.length, 0);

function gerarCodigo() {
  const l="ABCDEFGHJKLMNPQRSTUVWXYZ"; const r=()=>l[Math.floor(Math.random()*l.length)]; const n=()=>Math.floor(Math.random()*10);
  return `${r()}${r()}${r()}${n()}${n()}${n()}`;
}
...
function StickerChip({ code, count, onAction, locked }) {
  ...
  const flagSrc = flag(prefix);
  const icon = SPECIAL[prefix] || null;
  ...
  return (
    <div
      ...
      style={{
        background: C.bg, border: `2px solid ${C.bd}`, borderRadius: 8,
    ...
    >
      ...
      {flagSrc && (
        <img
          src={flagSrc}
          alt={prefix}
          style={{ width: 24, height: 18, objectFit: "cover", borderRadius: 2, marginBottom: 2, opacity: status === "m" ? 0.4 : 1 }}
        />
      )}
      {!flagSrc && icon && (
        <span style={{ fontSize: 13, lineHeight: 1, marginBottom: 2 }}>{icon}</span>
      )}
      <span style={{
        fontSize: 10, fontWeight: 700, color: C.tx,
        fontFamily: "'Courier New',monospace", textAlign: "center", lineHeight: 1,
      }}>{code}</span>
    </div>
  );
}
...
function Section({ section, stickers, onUpdate, search, filter, locked }) {
  ...
  const flagSrc = flag(section.id, "20x15");
  const icon    = SPECIAL[section.id] || null;
    ...
    <div style={{ marginBottom: 9 }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}>
          <span style={{
            fontSize: 11, color: "#94a3b8", flexShrink: 0, display: "inline-block",
...
function TrocasTab({ user, myStickers }) {
  ...
  const { possodar, possoReceber } = useMemo(() => {
    ...
    const minhasRep  = Object.entries(myStickers).filter(([,n])=>n>=2).map(([c])=>c);
    const minhasFalt = ALL_CODES.filter(c=>!myStickers[c]||myStickers[c]===0);
    const outroRep   = Object.entries(outroS).filter(([,n])=>n>=2).map(([c])=>c);
    const outroFalt  = ALL_CODES.filter(c=>!outroS[c]||outroS[c]===0);
    return { possodar: minhasRep.filter(c=>outroFalt.includes(c)), possoReceber: outroRep.filter(c=>minhasFalt.includes(c)) };
  ...
  const resultadoBusca = useMemo(() => {
    ...
    return ALL_CODES.filter(c=>c.includes(q)).slice(0,10).map(c => ({
      c, meuCount: myStickers[c]||0, outroCount: outroDados.stickers[c]||0,
      posso_dar: (myStickers[c]||0)>=2 && (outroDados.stickers[c]||0)===0,
      posso_receber: (outroDados.stickers[c]||0)>=2 && (myStickers[c]||0)===0,
    }));
  }, [busca, outroConectado, myStickers, outroDados]);

  const criarSala = async () => {
    const cod=gerarCodigo(), sen=gerarSenha();
    setLoading(true); setErro("");
    try {
      await setDoc(doc(db,"salas",cod), { codigo:cod, senha:sen, criadorId:user.uid, criadorNome:myName, criadorFoto:myPhoto, criadorStickers:myStickers, criadorOnline:true, convidadoId:null, convidadoNome:null, convidadoStickers:null, criadoEm:serverTimestamp() });
      setCodigoCriado(cod); setSenhaCriada(sen); setIsCriador(true);
      unsubRef.current = onSnapshot(doc(db,"salas",cod), snap=>{ if(snap.exists()) setSalaData(snap.data()); });
      setStep("aguardando");
    } catch { setErro("Erro ao criar sala."); }
    setLoading(false);
  };
  ...
  const entrarSala = async () => {
    ...
    try {
      ...
      await setDoc(doc(db,"salas",cod), { convidadoId:user.uid, convidadoNome:myName, convidadoFoto:myPhoto, convidadoStickers:myStickers, convidadoOnline:true }, {merge:true});
  ...
  const sairDaSala = async () => {
    ...
    try {
      const snap = await getDoc(doc(db,"salas",codigoCriado));
      if (snap.exists()) {
        const d = snap.data();
        if ((isCriador&&!d.convidadoId)||(d.criadorId&&d.convidadoId)) await deleteDoc(doc(db,"salas",codigoCriado));
        else await setDoc(doc(db,"salas",codigoCriado), isCriador?{criadorOnline:false}:{convidadoOnline:false}, {merge:true});
      }
    } catch {}
  ...
  const copiarInfo = () => {
    navigator.clipboard?.writeText(`🔄 Sala de Trocas — Figurinhas do Chico 2026\nCódigo: ${codigoCriado}\nSenha: ${senhaCriada}`);
    setCopiado(true); setTimeout(()=>setCopiado(false),2000);
  };

  const inStyle = { width:"100%", boxSizing:"border-box", background:"#1e293b", border:"1px solid #334155", borderRadius:10, padding:"12px 14px", color:"#e2e8f0", fontSize:15, outline:"none", marginBottom:10, fontFamily:"monospace", letterSpacing:2 };
  const btnStyle = (bg,extra={}) => ({ width:"100%", padding:"13px", background:bg, border:"none", borderRadius:10, color:"#fff", fontWeight:800, fontSize:14, cursor:"pointer", marginBottom:8, ...extra });
  ...
  if (step==="menu") return (
    <div style={{padding:"20px 16px"}}>
      <div style={{background:"#0f172a",borderRadius:12,padding:14,marginBottom:20}}>
        ...
        <div style={{display:"flex",gap:10}}>
          <div style={{flex:1,background:"#1e293b",borderRadius:8,padding:10,textAlign:"center"}}>
            <div style={{fontSize:22,fontWeight:900,color:"#f59e0b"}}>{Object.values(myStickers).filter(n=>n>=2).length}</div>
            <div style={{fontSize:9,color:"#64748b"}}>REPETIDAS</div>
          </div>
          <div style={{flex:1,background:"#1e293b",borderRadius:8,padding:10,textAlign:"center"}}>
            <div style={{fontSize:22,fontWeight:900,color:"#ef4444"}}>{ALL_CODES.filter(c=>!myStickers[c]||myStickers[c]===0).length}</div>
      ...
      <button onClick={criarSala} disabled={loading} style={btnStyle(loading?"#334155":"#6366f1")}>{loading?"Criando...":"➕ Criar sala"}</button>
      <button onClick={()=>setStep("entrar")} style={btnStyle("#0f172a",{border:"1px solid #334155"})}>🚪 Entrar em sala</button>
  ...
  if (step==="entrar") return (
    <div style={{padding:"20px 16px"}}>
      ...
      <input value={codigoInput} onChange={e=>setCodigoInput(e.target.value.toUpperCase())} placeholder="ex: CHI739" maxLength={8} style={inStyle}/>
      <div style={{fontSize:11,color:"#64748b",marginBottom:4,fontWeight:700}}>SENHA</div>
      <input value={senhaInput} onChange={e=>setSenhaInput(e.target.value)} placeholder="ex: 4821" maxLength={4} style={inStyle}/>
      {erro&&<div style={{color:"#ef4444",fontSize:12,marginBottom:10}}>⚠️ {erro}</div>}
      <button onClick={entrarSala} disabled={loading} style={btnStyle(loading?"#334155":"#6366f1")}>{loading?"Entrando...":"Entrar na sala"}</button>
    </div>
  );
  ...
  if (step==="aguardando") return (
    <div style={{padding:16}}>
      <div style={{background:"#0f172a",borderRadius:12,padding:14,marginBottom:12}}>
        ...
        <button onClick={copiarInfo} style={{width:"100%",padding:10,background:copiado?"#14532d":"#1e293b",border:copiado?"1px solid #22c55e":"1px solid #334155",borderRadius:10,color:copiado?"#86efac":"#94a3b8",fontWeight:700,fontSize:12,cursor:"pointer"}}>
      ...
      <button onClick={sairDaSala} style={btnStyle("#1e293b",{border:"1px solid #ef4444",color:"#ef4444"})}>Cancelar e apagar sala</button>
    </div>
  );
    ...
    <div style={{padding:16}}>
      <div style={{background:"#0f172a",borderRadius:12,padding:"12px 14px",marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <div style={{flex:1,background:"#1e293b",borderRadius:8,padding:8,display:"flex",alignItems:"center",gap:6}}>
            {myPhoto&&<img src={myPhoto} style={{width:24,height:24,borderRadius:"50%",objectFit:"cover"}}/>}
            <div><div style={{fontSize:10,color:"#22c55e",fontWeight:700}}>● Você</div><div style={{fontSize:10,color:"#94a3b8",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:80}}>{myName}</div></div>
          </div>
          <div style={{flex:1,background:"#1e293b",borderRadius:8,padding:8,display:"flex",alignItems:"center",gap:6}}>
            {outroDados?.foto&&<img src={outroDados.foto} style={{width:24,height:24,borderRadius:"50%",objectFit:"cover"}}/>}
            <div>
              <div style={{fontSize:10,color:outroConectado?"#22c55e":"#f59e0b",fontWeight:700}}>{outroConectado?`● ${outroDados?.nome?.split(" ")[0]}`:"⏳ Aguardando..."}</div>
      ...
      {outroConectado ? (<>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
          <div style={{background:"#14532d",border:"1px solid #22c55e",borderRadius:10,padding:10,textAlign:"center"}}>
          ...
          <div style={{background:"#1e3a5f",border:"1px solid #3b82f6",borderRadius:10,padding:10,textAlign:"center"}}>
        ...
        <div style={{background:"#0f172a",borderRadius:12,padding:12,marginBottom:12}}>
          <div style={{fontSize:11,fontWeight:700,color:"#94a3b8",marginBottom:8,textTransform:"uppercase"}}>🔍 Verificar figurinha</div>
          <div style={{position:"relative"}}>
            <input value={busca} onChange={e=>setBusca(e.target.value.toUpperCase())} placeholder="ex: BRA3, FWC2..."
              style={{width:"100%",boxSizing:"border-box",background:"#1e293b",border:"1px solid #334155",borderRadius:9,padding:"9px 30px 9px 12px",color:"#e2e8f0",fontSize:13,outline:"none",fontFamily:"monospace"}}/>
          ...
          {resultadoBusca&&resultadoBusca.length>0&&(
            <div style={{marginTop:8,display:"flex",flexDirection:"column",gap:5}}>
              {resultadoBusca.map(({c,meuCount,outroCount,posso_dar,posso_receber})=>(
                <div key={c} style={{background:posso_dar?"#14532d":posso_receber?"#1e3a5f":"#1e293b",border:`1px solid ${posso_dar?"#22c55e":posso_receber?"#3b82f6":"#334155"}`,borderRadius:8,padding:"9px 12px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <div>
                    <span style={{fontSize:13,fontWeight:900,fontFamily:"monospace",color:posso_dar?"#86efac":posso_receber?"#93c5fd":"#94a3b8"}}>{c}</span>
                    <span style={{fontSize:10,marginLeft:8,color:posso_dar?"#86efac":posso_receber?"#93c5fd":"#64748b"}}>{posso_dar?"💚 pode dar":posso_receber?"💛 pode pedir":"sem troca"}</span>
                  </div>
                  <div style={{display:"flex",gap:8,fontSize:11}}>
                    <span style={{color:meuCount>=2?"#f59e0b":meuCount===1?"#22c55e":"#ef4444",fontWeight:700}}>eu ×{meuCount}</span>
                    <span style={{color:outroCount>=2?"#f59e0b":outroCount===1?"#22c55e":"#ef4444",fontWeight:700}}>ele ×{outroCount}</span>
                  </div>
                </div>
              ))}
            </div>
          )}
          {!busca&&<div style={{marginTop:6,fontSize:11,color:"#475569",textAlign:"center"}}>Digite o código para verificar qualquer figurinha</div>}
        </div>

        {possodar.length>0&&(
          <div style={{background:"#0f172a",borderRadius:12,padding:12,marginBottom:10}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
              {possodar.map(c=><div key={c} style={{background:"#14532d",border:"1px solid #22c55e",borderRadius:6,padding:"4px 10px",fontSize:11,fontWeight:700,color:"#86efac",fontFamily:"monospace"}}>{c} ×{myStickers[c]}</div>)}
            </div>
          </div>
        )}
        {possoReceber.length>0&&(
          <div style={{background:"#0f172a",borderRadius:12,padding:12,marginBottom:10}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
              {possoReceber.map(c=><div key={c} style={{background:"#1e3a5f",border:"1px solid #3b82f6",borderRadius:6,padding:"4px 10px",fontSize:11,fontWeight:700,color:"#93c5fd",fontFamily:"monospace"}}>{c}</div>)}
            </div>
          </div>
        )}
...
async function gerarPDF(stickers, userName) {
  ...
  const statsY = 30;
  [
    { label: "Completado", val: `${pct}%`,          color: PURPLE },
    { label: "Tenho",      val: have,                color: GREEN  },
    { label: "Faltam",     val: faltam.length,       color: RED    },
    { label: "Repetidas",  val: repetidas.length,    color: ORANGE },
  ].forEach((s, i) => {
    const x = 10 + i * 48;
    doc.setFillColor(...LGRAY);
    doc.roundedRect(x, statsY, 44, 14, 2, 2, "F");
  ...
  const drawBlock = (title, items, countFn, startY, titleColor, chipBorder, chipText) => {
    ...
    if (items.length === 0) {
      ...
      doc.setFontSize(8);
      doc.setFont("helvetica", "normal");
      doc.text("Nenhuma figurinha nesta categoria.", 10, startY + 15);
      return startY + 20;
    }
    ...
    items.forEach(code => {
      ...
      doc.setFontSize(5.5);
      doc.setFont("helvetica", "bold");
      const label = count > 1 ? `${code} x${count}` : code;
      doc.text(label, cx + chipW/2, cy + 4.5, { align: "center" });

      col++;
      if (col >= cols) { col = 0; cx = 10; cy += chipH + gap; }
      else { cx += chipW + gap; }
    });

    const rows = Math.ceil(items.length / cols);
    return startY + 11 + rows * (chipH + gap) + 6;
  };
...
export default function App() {
  ...
  const handleUpdate = useCallback((code, type) => {
    if (locked) return;
    setStickers(prev => {
      const n=prev[code]||0, next=type==="tap"?n+1:0, upd={...prev,[code]:next};
      saveToCloud(upd);
  ...
  const handleExport = () => {
    const a = Object.assign(document.createElement("a"), {
      href: URL.createObjectURL(new Blob([JSON.stringify({versao:"copa26_firebase",data:new Date().toLocaleDateString("pt-BR"),figurinhas:stickers},null,2)],{type:"application/json"})),
      download: `figurinhas-chico-backup-${new Date().toISOString().slice(0,10)}.json`
    });
  ...
  const handleImport = (e) => {
    ...
    r.onload=(ev)=>{ try{ const fig=(JSON.parse(ev.target.result).figurinhas||JSON.parse(ev.target.result)); setStickers(fig); saveToCloud(fig); showToast("✅ Backup restaurado!"); setShowBackup(false); }catch{ showToast("❌ Arquivo inválido!"); } };
    r.readAsText(file); e.target.value="";
  };

  const teamStats = useMemo(() =>
    ALBUM_OFICIAL.filter(s=>s.group).map(s=>{ const h=s.stickers.filter(c=>(stickers[c]||0)>0).length; return{...s,have:h,pct:Math.round(h/s.stickers.length*100)}; }).sort((a,b)=>b.pct-a.pct)
  , [stickers]);
          ...
          <div style={{padding:9}}>
            ...
            {teamStats.map(s=>(
              <div key={s.id} style={{display:"flex",alignItems:"center",gap:7,marginBottom:6,background:"#0f172a",borderRadius:9,padding:"7px 10px"}}>
                ...
                {flag(s.id,"20x15")&&<img src={flag(s.id,"20x15")} alt={s.id} style={{width:22,height:16,objectFit:"cover",borderRadius:2,flexShrink:0}}/>}
