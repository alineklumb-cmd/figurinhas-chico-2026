...
import { ALL_DATA, ALBUM_OFICIAL, EXTRA_SECTION, TOTAL_OFICIAL } from "./albumData";
...
// ─── CHIP ─────────────────────────────────────────────────────────────────────
function StickerChip({ code, count, onAction }) {
  ...
  const start = () => { pressRef.current = setTimeout(() => { pressRef.current = null; onAction("long"); }, 500); };
  const end   = (e) => { e.preventDefault(); if (pressRef.current) { clearTimeout(pressRef.current); pressRef.current = null; onAction("tap"); } };
  return (
    <div onTouchStart={start} onTouchEnd={end} onClick={() => onAction("tap")}
      style={{ background:C.bg, border:"2px solid "+C.bd, borderRadius:8,
        ...
        cursor:"pointer", userSelect:"none", position:"relative", minHeight:40,
...
// ─── SEÇÃO ────────────────────────────────────────────────────────────────────
function Section({ section, stickers, onUpdate, search, filter }) {
    ...
    <div style={{ marginBottom:9 }}>
      <div onClick={() => setOpen(p=>!p)}
        ...
        <div style={{ display:"flex", alignItems:"center", gap:6, minWidth:0 }}>
          <span style={{ fontSize:11, color:"#94a3b8", flexShrink:0, display:"inline-block", transition:"transform 0.2s",
...
export default function App() {
  ...
  // Firestore listener — sincroniza em tempo real
  useEffect(() => {
    if (!user) return;
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#020617", fontFamily:"'Segoe UI',system-ui,sans-serif",
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:9, color:"#475569" }}>🏆 48 seleções · 980 figurinhas oficiais</div>
            </div>
          </div>
          <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:3 }}>
            <div style={{ fontSize:20, fontWeight:900, color:"#6366f1", fontFamily:"monospace" }}>{stats.pct}%</div>
            <div style={{ display:"flex", alignItems:"center", gap:4 }}>
              {saving && <span style={{ fontSize:8, color:"#64748b" }}>💾</span>}
              <button onClick={() => signOut(auth)}
                style={{ fontSize:8, color:"#475569", background:"#1e293b", border:"none",
        ...
        {tab === "album" && <>
          <div style={{ position:"relative", marginBottom:7 }}>
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)}
              placeholder="ex: BRA1, FRA5, FWC3, ES1G..."
      ...
      <div style={{ padding:"9px 9px 0" }}>
        {tab === "album" && ALL_DATA.map(s => (
          <Section key={s.id} section={s} stickers={stickers}
            onUpdate={handleUpdate} search={search} filter={filter}/>
        ))}
        {tab === "stats" && (
          <div>
      ...
      <div style={{ position:"fixed", bottom:0, left:0, right:0, background:"#0c1422",
        ...
        <span style={{ fontSize:8, color:"#334155", alignSelf:"center" }}>• Segure → remover</span>
      </div>
